...
def measure_inference_speed(cfg, checkpoint, max_iter, log_interval,
    ...
    ######## adding by myself ###########
    print('inf_time_save_path:', inftime_save_path)
    with open (inftime_save_path + '/inference_time.txt','w') as f:
        f.write(f'Overall fps: {fps:.1f} img / s, times per image: {1000 / fps:.1f} ms / img')
    return fps
...
def main():
    global inftime_save_path
    args = parse_args()
    inftime_save_path = args.save_path
    ...
    test = repeat_measure_inference_speed(cfg, args.checkpoint, args.max_iter,
