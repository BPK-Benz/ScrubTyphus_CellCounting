...
class CocoCellsImage:
	def __init__(self, index):
		...
		# load image
		image_path = coco['images'][index]['file_name'].replace('Testing', 'Testing') # replace('Testing', 'Testing_set')
		print(coco['images'][index]['file_name'])
	...
	def draw(self):
		...
		if mode == 'overlay':
			...
			image[ch2 > 127] = (0, 200, 0) # 255 to 200
...
if __name__ == "__main__":
	...
	# load coco data
	coco_path = os.path.join(root, "Coco_File/InfectTotal_TestNuc_3class.json")
	# coco_path = os.path.join(root, "data/output_coco/S1/Plate_03/NucleusCell_Testing/006018-8-001001.json")
	# coco_path = '001001-1-001001002.json'
	# coco_path = 'test.json'
	coco = read_coco()
