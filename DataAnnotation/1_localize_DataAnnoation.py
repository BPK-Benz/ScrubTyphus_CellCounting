...
class cellPlate: # managing folders for source and target 
    def __init__(self, navigation, navigation_cp):

        # get absolute
        self.root_path = os.path.dirname(os.path.realpath(__file__))
        self.navigation = navigation
        self.navigation_cp = navigation_cp
    
        # input paths
        self.original_path = os.path.join("r", self.root_path, "data_old/Enhance_image", self.navigation)
        self.cellprofiler_path = os.path.join("r", self.root_path, "data_old/Output_CP", self.navigation_cp)

        # output paths, hint: put "r" to usable all os for reading pathway
        self.localize_path = os.path.join("r", self.root_path, "data_old/output_localize", self.navigation)
        self.image_path = os.path.join("r", self.root_path, "data_old/output_image", self.navigation)
        self.segment1_path = os.path.join("r", self.root_path, "data_old/output_segment1", self.navigation)
        self.segment3_path = os.path.join("r", self.root_path, "data_old/output_segment3", self.navigation)
        self.coco_path = os.path.join("r", self.root_path, "data_old/output_coco", self.navigation)
        self.properties_path = os.path.join("r", self.root_path, "data_old/output_properties", self.navigation)
        ...
        self.bacteria_profiles = pd.read_csv(os.path.join(self.cellprofiler_path, 'Image_Processing_Bac.csv'))
        self.nucleus_profiles = pd.read_csv(os.path.join(self.cellprofiler_path, 'Image_Processing_Nucleus.csv'))
        self.cytoplasm_profiles = pd.read_csv(os.path.join(self.cellprofiler_path, 'Image_Processing_Cell.csv'))
        ...
        self.is_auto = True   # If is_export is True, is_auto should be True as well to automatic changing images (every change images are saved)
    ...
    def load_image(self):
        ...
        # print(self.local2_path)
        # print(cellprofiler2_path)
        ...
        if os.path.exists(self.local3_path):
            self.load_cytoplasm()
            # print(self.local3_path)
        else:
            self.make_cytoplasm()  # For only the first time
            print('Nooooooooo')
            print(self.local3_path, self.local2_path)
    ...
    def make_nucleus(self): # to find the location and contour of nucleus from result of cellprofiler
        ...
        # skip if nucleus less than 200 and more than 80000 pixel
        skips = []
...
def onKeyPress(event):
    ...
    if image.mode in ['mark']:
        ...
        # Change channel
            ...
            image.display_mark()
        elif event.keysym == 'F2': #F2
            image.channel = 'nucleus'
            image.display_mark()
        elif event.keysym == 'F3':#F3
            image.channel = 'cell'
            image.display_mark()
...
if __name__ == "__main__":
    ...
    navigation = 'S1/Plate_03/Testing' 
    navigation_cp = 'S1/Plate_03/Gene'
    print(navigation, navigation_cp)
    image = cellPlate(navigation, navigation_cp)
    ...
    # Scramble Split_scramble Testing Split_testing
    # Scramble Gene
    # Plate_03 Plate_04 Plate_07